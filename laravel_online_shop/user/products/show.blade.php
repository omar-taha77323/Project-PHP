...
<main class="flex flex-col items-center py-6 px-4 md:px-10">
  <div class="max-w-[1200px] w-full flex flex-col">
    ...
    <nav class="flex flex-wrap gap-2 py-4 mb-2">
      <a class="text-[#617c89] dark:text-gray-400 text-sm font-medium hover:text-primary" href="{{ route('user.products.index') }}">Shop</a>
    ...
    <div class="grid grid-cols-12 gap-8 bg-white dark:bg-background-dark p-6 md:p-8 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800">
      ...
      <div class="col-span-12 lg:col-span-7 flex flex-col gap-4">
        <div class="w-full aspect-[4/3] rounded-xl overflow-hidden bg-[#f0f3f4] dark:bg-gray-800">
          <div class="w-full h-full bg-center bg-no-repeat bg-cover" style="background-image:url('{{ $imgUrl }}')"></div>
        </div>
          ...
          <div class="grid grid-cols-5 gap-3">
            @foreach($images->take(5) as $img)
              ...
              <a href="{{ $thumb }}" target="_blank"
                ...
                <div class="w-full h-full bg-center bg-no-repeat bg-cover" style="background-image:url('{{ $thumb }}')"></div>
              </a>
            @endforeach
          </div>
        @endif
      ...
      <div class="col-span-12 lg:col-span-5 flex flex-col">
        ...
        <div class="flex items-center gap-4 mb-6">
          <span class="text-3xl font-bold text-[#111618] dark:text-white">${{ number_format($price, 2) }}</span>
        ...
        <div class="flex flex-col gap-4 mt-auto">
          ...
          {{-- Wishlist --}}
          <form action="{{ route('user.wishlist.toggle', $product) }}" method="POST" class="flex gap-3">
            @csrf
            <button class="flex-1 flex items-center justify-center gap-2 border border-gray-200 dark:border-gray-700 py-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors text-sm font-medium h-12">
              <span class="material-symbols-outlined text-[20px] {{ $inWishlist ? 'text-red-500' : '' }}">favorite</span>
    ...
    @if($related->count())
      <div class="mt-10">
        <div class="flex items-center justify-between mb-4">
          ...
          <a href="{{ route('user.products.index') }}" class="text-primary text-sm font-semibold hover:underline">Back to shop</a>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          @foreach($related as $p)
            ...
            @endphp
            <a href="{{ route('user.products.show', $p) }}" class="group bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden hover:shadow-xl transition-all">
              <div class="relative aspect-square overflow-hidden bg-gray-100">
                <img src="{{ $rimg }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" alt="{{ $p->name }}">
              </div>
              <div class="p-4">
                <div class="text-sm font-bold text-[#111618] dark:text-white line-clamp-1">{{ $p->name }}</div>
