...
<main class="flex-1 w-full max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-10 py-8">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
        <div class="space-y-2">
            ...
            <p class="text-[#617c89] dark:text-gray-400 text-lg">
                Browse through our curated collections ({{ method_exists($categories, 'total') ? $categories->total() : $categories->count() }} Available)
            </p>
        </div>
    ...
    <div class="mt-16 flex flex-col items-center justify-center gap-6 border-t border-[#f0f3f4] dark:border-white/10 pt-10 pb-20">
        @php
            // في حالة paginate(): عندنا firstItem/lastItem/total
            $isPaginated = method_exists($categories, 'total');
        @endphp

        @if($isPaginated && $categories->total() > 0)
