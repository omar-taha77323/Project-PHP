    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
        ...
        <div class="flex flex-col lg:flex-row gap-8">
                ...
                <form method="GET" action="{{ route('user.products.index') }}"
                    ...
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <a href="{{ route('user.products.index') }}"
                            class="text-primary text-xs font-semibold hover:underline">Reset</a>
                    </div>
                    ...
                    {{-- Categories --}}
                    <div class="space-y-3">
                        <h4 class="text-gray-900 dark:text-white text-sm font-semibold uppercase tracking-wider">Categories
                        </h4>
                        <div class="space-y-2">
                            @foreach ($categories as $cat)
                                ...
                                <label class="flex items-center justify-between group cursor-pointer">
                                    <div class="flex items-center gap-3">
                                        <input name="categories[]" value="{{ $cat->id }}" @checked($checked)
                                            class="h-5 w-5 rounded border-gray-300 dark:border-gray-700 text-primary focus:ring-primary checkbox-custom"
                                            type="checkbox" />
                                        <span
                                            class="text-gray-600 dark:text-gray-400 text-sm group-hover:text-primary transition-colors {{ $checked ? 'font-medium' : '' }}">
                                            {{ $cat->name }}
                                        </span>
                                    </div>
                    ...
                    {{-- Price Range --}}
                    <div>
                        <h4 class="text-gray-900 dark:text-white text-sm font-semibold uppercase tracking-wider mb-4">Price
                            Range</h4>
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="text-xs text-gray-500">Min</label>
                                <input name="min_price" value="{{ request('min_price') }}" type="number" step="0.01"
                                    min="0"
                            ...
                            <div>
                                ...
                                <input name="max_price" value="{{ request('max_price') }}" type="number" step="0.01"
                                    min="0"
                                    class="mt-1 block w-full border-none bg-gray-100 dark:bg-gray-800 rounded-lg text-sm placeholder-gray-500 focus:ring-2 focus:ring-primary dark:text-gray-200"
                                    placeholder="9999" />
                            </div>
                    ...
                    {{-- Brands --}}
                    <div>
                        <h4 class="text-gray-900 dark:text-white text-sm font-semibold uppercase tracking-wider mb-4">Brands
                        </h4>
                        <div class="space-y-2">
                            @foreach ($brands as $b)
                                @php
                                ...
                                <label class="flex items-center gap-3 cursor-pointer">
                                    <input name="brands[]" value="{{ $b->id }}" @checked($bChecked)
                    ...
                    {{-- Apply --}}
                    <button
                        class="w-full bg-primary hover:bg-primary/90 text-white font-bold py-3 rounded-lg transition-colors flex items-center justify-center gap-2">
            ...
            <div class="flex-1">
                ...
                {{-- Toolbar --}}
                <div
                    class="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4 bg-white dark:bg-gray-900 p-4 rounded-xl border border-gray-200 dark:border-gray-800">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    ...
                    @forelse($products as $p)
                        @php
                            ...
                            // ✅ Wishlist state (Session)
                            $wishlist = session('wishlist', []);
                            $inWishlist = in_array($p->id, $wishlist);
                        @endphp

                        <div
                            class="group bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden hover:shadow-xl transition-all duration-300">
                            <a href="{{ route('user.products.show', $p->id) }}"
                                class="block relative aspect-square overflow-hidden bg-gray-100">
                                @if ($img)
                                ...
                                @if ($hasSale)
                                    <div
                                        class="absolute top-3 left-3 bg-red-500 text-white text-[10px] font-bold px-2 py-1 rounded uppercase">
                                        SALE
                                    </div>
                                @endif

                                {{-- ✅ wishlist toggle (FIXED: use $p not $product) --}}
                                <form method="POST" action="{{ route('user.wishlist.toggle', $p->id) }}">
                                    @csrf
                                    <button type="submit"
                                        class="absolute top-3 right-3 w-10 h-10 rounded-full flex items-center justify-center
                            ...
                            <div class="p-5">
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <form method="POST" action="{{ route('user.cart.add', $p->id) }}">
                                        @csrf
                                        <button
                                            class="p-3 bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white rounded-lg hover:bg-primary hover:text-white transition-colors"
                                            type="submit">
